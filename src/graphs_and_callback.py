...
def timeseries(period):
    ...
    fig.update_layout(
        **update_layout_simple,
        ...
        legend=dict(x=.013, y=1, title=None),
        xaxis=dict(
            showgrid=False,
            dtick="M1",
            tickformat="%B",
...
def distribution_ev_fat(bin_ev, bin_fat):
    ...
    def dist(val, bin_size, name, color):
        hist_data = [val]
        group_labels = [name]
        ...
        fig.update_layout(
            **update_layout_simple,
            ...
            title={
                "text": (
                    f"<b>Frequency of events and<br>fatalities in europe</b><br />"
                    f"<sup style='color:silver'>All natural disasters occurred: 1970 - 2023"
                ),
                "font": {"family": "serif", "size": 20, "color": "white"},
                "x": 0.98,
...
def distribution_conflict_ue():
    ...
    fig.update_layout(
        **update_layout_simple,
        ...
        title={
            "text": (
                f"<b>Monthly Trend of<br>Natural Disasters</b><br />"
                f"<sup style='color:silver'>All natural disasters occurred: 1970 - 2023"
            ),
            "font": {"family": "serif", "size": 20, "color": "white"},
            "x": 0.98,
